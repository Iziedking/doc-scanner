...
The free version is supported by ads. They never see your documents.
...
The app serves AdMob ads to free users, so it does collect and share data.
Declare it. A "we collect nothing" answer alongside an ads SDK is the fast
route to a rejection or a takedown.

Answer as follows:

- Does your app collect or share any of the required user data types? **Yes**
- **Device or other IDs** (advertising ID): collected and **shared**.
  Purpose: Advertising or marketing. Is it required? No, ads run only in
  the free tier and consent is requested where the law demands it.
- **App activity, in-app search history**: not collected. Search runs
  entirely on the device.
- **Photos** and **Files and docs**: not collected. Scans stay on the
  device.
- Is all of the user data collected by your app encrypted in transit? Yes.
  AdMob and RevenueCat both use HTTPS.
- Do you provide a way for users to request that their data is deleted?
  Documents are local; deleting them or uninstalling removes everything.
  The advertising ID can be reset from Android settings.

Once RevenueCat billing is live, add:

- **Purchase history**: collected and shared with a service provider
  (RevenueCat processes the purchase), not used for tracking.

Use the optional free-text field to say plainly: documents, page images,
and recognized text never leave the device.

## Ads declaration

Play asks separately whether the app contains ads. Answer **Yes**. The
listing then shows the "Contains ads" label, which is required and is not
optional once an ads SDK is present.
